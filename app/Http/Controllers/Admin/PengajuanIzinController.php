...
{
    // Untuk panggil view
    private $views      = 'admin/pengajuan/pengajuan_izin';
    
    // Untuk keperluan redirect, hubungannya route / file web
    private $url        = 'admin/pengajuan_izin';
    ...
    {
        $pengajuan_izin     = $this->mPengajuanIzin->where('idPIzin', $request->id)->first();
        ...
        $dataPengajuanIzin = [
            ...
            'supervisor'        => $pengajuan_izin->supervisor,
        ];
        ...
        $templateProcessor->setValue('supervisor', $dataPengajuanIzin['supervisor']);
    ...
    public function edit($id)
    {
        ...
        $data = [
            'title'                 => $this->title,
            'url'                   => $this->url,
            'page'                  => 'Verifikasi Pengajuan Izin Keluar Masuk',
            'pengajuan_izin'        => $pengajuan_izin,
            'jabatan'               => $jabatan,
            'area'                  => $area,
        ];
    ...
    public function update(Request $request, $id)
    {
        ...
        if($request->status == 2 OR $request->status == 3){
            ...
            // echo json_encode($dataPengajuanIzin); die();
            $this->mPengajuanIzin->where('idPIzin', $id)->update($dataPengajuanIzin);
            ...
            // echo json_encode($dataPengajuanIzin); die();
            $this->mPengajuanIzin->where('idPIzin', $id)->update($dataPengajuanIzin);
            return redirect("$this->url")->with('sukses', 'Data Izin Keluar Masuk berhasil di edit');
        }
